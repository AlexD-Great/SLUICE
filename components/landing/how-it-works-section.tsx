...
const steps = [
  {
    number: "01",
    title: "Request",
    subtitle: "from any language",
    description: "Your agent POSTs a Filecoin Pay operation. No wallet, no private key, no JavaScript runtime — just an HTTP call and an API key.",
    code: `r = requests.post(
  f"{SLUICE}/api/v1/pay/authorize",
  headers={"Authorization": f"Bearer {KEY}"},
  json={"kind": "deposit", "amount": "0.25"},
)
# 200 — under cap, already on-chain`,
  },
  {
    number: "02",
    title: "Gate",
    subtitle: "on the spend cap",
    description: "Sluice prices the request against the chain first, then compares it to the cap on the calling key. Over the cap it is held, and nothing is signed.",
    code: `json={"kind": "deposit", "amount": "12.0"}
# 202 Accepted
{
  "status": "pending_approval",
  "amount": {"usdfc": "12.0", "basis": "stated"},
  "capUsdfc": "1"
}`,
  },
  {
    number: "03",
    title: "Release",
    subtitle: "when a human says so",
    description: "The held request appears in the control room in real time. One click signs it, and the caller's poll loop picks up the receipt seconds later.",
    code: `while True:
  s = requests.get(
    f"{SLUICE}/api/v1/pay/status/{id}",
    headers=auth).json()["authorization"]
        ...
        <div className="relative mb-0 lg:mb-0 grid lg:grid-cols-2 gap-4 lg:gap-12 items-end">
          ...
          <div className="overflow-hidden pb-0 lg:pb-32">
            ...
            <h2 className={`text-6xl md:text-7xl lg:text-[128px] font-display tracking-tight leading-[0.85] transition-all duration-1000 delay-100 ${
              ...
              <span className="block text-white/10">Release.</span>
            </h2>
          </div>

...
const regions = [
  { name: "POST /pay/authorize", nodes: 5, status: "operational" },
  { name: "POST /pay/approve/:id", nodes: 1, status: "operational" },
  { name: "GET /pay/status/:id", nodes: 1, status: "operational" },
  { name: "GET /verify/:pieceCid", nodes: 1, status: "operational" },
];
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg transition-all duration-1000 delay-100 ${
                ...
                Four endpoints and a health check. Everything else &mdash; provider selection,
                PieceCID computation, rail creation, proof timing &mdash; happens inside Sluice.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-8xl lg:text-[10rem] font-display leading-none">5</span>
                <span className="text-2xl text-muted-foreground">endpoints</span>
              </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-100 ${
              ...
              <span className="text-5xl lg:text-6xl font-display">5</span>
              <span className="block text-sm text-muted-foreground mt-2">Filecoin Pay operations brokered</span>
            </div>
            ...
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-200 ${
              ...
              <span className="block text-sm text-muted-foreground mt-2">transaction signed at a time</span>
            </div>
          </div>
        </div>
        ...
        <div className={`mt-12 grid grid-cols-2 lg:grid-cols-4 gap-4 transition-all duration-1000 delay-300 ${
          ...
          {regions.map((region, index) => (
            ...
            >
              ...
              <span className="text-sm text-muted-foreground">gated</span>
            </div>
          ))}
        </div>
      </div>

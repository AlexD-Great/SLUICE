...
/**
 * Fallbacks only. The section swaps these for live counters from
 * /api/v1/stats on mount, so the numbers on the marketing page are the same
 * ones the gateway actually recorded.
 */
const metrics = [
  {
    value: 0,
    suffix: "",
    prefix: "",
    label: "Payments brokered",
    sublabel: "through Filecoin Pay",
  },
  {
    value: 0,
    suffix: "",
    ...
    sublabel: "over the spend cap",
  },
  {
    value: 0,
    suffix: "",
    prefix: "",
    label: "PDP proofs checked",
    sublabel: "via GET /verify",
  },
];
...
export function MetricsSection() {
  ...
  // Real counters from the gateway. If the API is unreachable the fallbacks
  // above stay on screen rather than the section breaking.
  useEffect(() => {
    fetch("/api/v1/stats")
      .then((response) => response.json())
      .then((stats) => {
        setLive([
          { ...metrics[0], value: stats.payments?.executed ?? 0 },
          { ...metrics[1], value: stats.payments?.gated ?? 0 },
          { ...metrics[2], value: stats.proofChecks?.total ?? 0 },
        ]);
      })
      .catch(() => {});
  }, []);
    ...
    <section ref={sectionRef} className="relative py-32 lg:py-40 overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-1 bg-foreground/[0.02] border border-foreground/10 p-10 lg:p-14 transition-all duration-700 ${
            ...
            <div className="text-4xl md:text-5xl lg:text-6xl font-display tracking-tight mb-4 whitespace-nowrap overflow-hidden">
              <AnimatedNumber end={live[0].value} suffix={live[0].suffix} prefix={live[0].prefix} />
            </div>
            <div className="mb-6">
              <DotGraph color="white" height={36} freq1={0.28} freq2={0.09} freqT={0.5} speed={0.018} baseline={0.35} amplitude={0.55} />
            </div>
            <div className="text-lg text-foreground mb-2">{live[0].label}</div>
            <div className="text-sm text-muted-foreground font-mono">{live[0].sublabel}</div>
          </div>

          {/* Metrics */}
          {live.slice(1).map((metric, index) => (
            <div
              key={metric.label}
        ...
        <div className={`mt-16 pt-8 border-t border-foreground/10 flex flex-wrap items-center gap-x-12 gap-y-4 text-sm font-mono text-muted-foreground transition-all duration-1000 delay-500 ${
          ...
          <span>PDP / Warm Storage</span>
          <span>USDFC</span>
          <span>Synapse SDK</span>
          <span className="text-foreground">Calibration testnet</span>
        </div>
      </div>
    </section>
  );
